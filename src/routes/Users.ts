...
export async function addNotificationByFullName(req: Request, res: Response) {
  ...
  if (!query.firstname || !query.lastname) {
    ...
    if (user) {
      const mUser = <IUser>user.toObject();
...
export async function uploadPP(req: Request, res: Response) {
  const queryResult = await UserModel.findById(req.body._id);
  if (queryResult) {
...
export async function transferMoney(req: Request, res: Response) {
  ...
  if (mUser) {
    const mUserObject: IUser = mUser.toObject();
    ...
    const [firstnameR, lastnameR] = transfer.user.split(".");
    const mUserR = await UserModel.findOne({
      lastname: lastnameR,
      firstname: firstnameR,
    });
    if (mUserR) {
      const mUserRObject: IUser = mUser.toObject();
